...
The college GPA is the response variable and is labeled on the vertical axis.  The scatterplot in @fig-sp1 shows that the college GPA increases as the high school GPA increases.  In fact, the the dots appear to cluster along a straight line.  The correlation coefficient is $r = `r cor(Gpa$hsgpa, Gpa$collgpa)`$, which indicates that a straight line is a reasonable relationship between the two variables.
