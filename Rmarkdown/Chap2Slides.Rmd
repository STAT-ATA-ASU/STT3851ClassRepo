---
title: "Chapter Two Graphs and More"
...
\begin{equation}
\text{CI}_{1 - \alpha}(\beta_1) = \left[b_1 - t_{1- \alpha/2, n - p }SE(b1), b_1 + t_{1- \alpha/2, n - p}SE(b1) \right]
(\#eq:ci)
\end{equation}
...
**Example** Use the `GRADES` data set from the `PASWR2` package and model `gpa` as a function of `sat`.  Compute the expected GPA (`gpa`) for an SAT score (`sat`) of 1300.  Construct a 90% confidence interval for the mean GPA for students scoring 1300 on the SAT.
...
Credit %>% 
  group_by(Gender) %>% 
  summarize(AvgBalnace = mean(Balance))
#
library(ggplot2)
...
NM <- S2M %>% 
  augment()
head(NM)
ggplot(data = NM, aes(x = Limit, y = Balance, color = Student)) + 
  geom_point() + 
...
4. Use your model to predict the `Rating` for an individual who has a credit card limit of
