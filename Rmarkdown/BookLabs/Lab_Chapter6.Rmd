...
We find the best model is the one that contains ten variables.
...
subset selection _within each of the k training sets_. Despite this, we see that
...
Now we write a for loop that performs cross-validation. In the $j^{th}$ fold, theelements of `folds` that equal `j` are in the test set, and the remainder are in the training set. We make our predictions for each model size (using our new `predict()` method), compute the test errors on the appropriate subset, and store them in the appropriate slot in the matrix `cv.errors`.
...
myControl <- trainControl(## 10-fold CV
                           ...
                           repeats = 10)
set.seed(314)
regfit_fwd <- train(Salary ~ ., data = Hitters,
                    method = "leapForward",
                    trControl = myControl,
                    # tuneLength = 15
...
regfit_fwd
head(predict(regfit_fwd, newdata = Hitters[test,]))
### Note that the following is the same output
coef(reg.best, 9)
head(predict(reg.best, id= 9, newdata = Hitters[test,]))
